...
export default function Header() {
    ...
    const isActiveLink = href => {
        return router.asPath === href ? "active-link" : ""
    }
    ...
    return (
        <header className="flex flex-col sm:flex-row w-full items-center container mx-auto py-3 sm:py-5 xl:py-5 px-3 sm:px-5 xl:px-0 containers justify-between sm:justify-start">
            ...
            <div className="z-50 w-full xl:hidden flex justify-between items-center">
                ...
                <Link href="/search">
                    <a>
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-white mr-4 transform transition duration-300 hover:text-gray-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <Transition
                ...
                leaveTo="opacity-0"
                className="absolute z-40 bg-black w-screen h-screen flex flex-col items-center mt-14 pt-14 sm:mt-180 pb-28 border-b border-gray-800"
            >
                <Link href="/movie">
                    <a className={`${isActiveLink("/movie")} text-2xl font-title mb-3 transform transition duration-300 hover:text-gray-300`}>Movies</a>
                </Link>
                <Link href="/tv">
                    <a className={`${isActiveLink("/tv")} text-2xl font-title mb-10 transform transition duration-300 hover:text-gray-300`}>TV Shows</a>
                </Link>
            </Transition>
                    ...
                    >   
                        <div className="flex flex-col">                        
                            <input 
                                type="text" 
                                placeholder="Search" 
                                className="w-60 text-lg font-title bg-black focus:outline-none" 
                                onChange={handleSearchChange}                                                            
                                value={value}
                            />                            
                            {
                                searchActive && searchResults !== undefined && (
                                    <ul className="w-60 bg-black font-title absolute z-50 mt-9 border border-truegray-800 rounded">
