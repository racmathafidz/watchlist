...
export default function TvDetail(props) {
  ...
  const {
    tvDetailData, tvVideoData, tvCreditData, tvSimilarData,
  } = props;
...
export const getServerSideProps = async (paths) => {
  ...
  const filteredTvVideoData = await tvVideoData.results.filter((items) => items.site === 'YouTube');
  ...
  const filteredTvCreditData = await tvCreditData.cast.filter((items) => items.known_for_department === 'Acting' && items.profile_path !== null);
  ...
  const filteredTvSimilarData = await tvSimilarData.results.filter((items) => items.backdrop_path !== null && items.poster_path !== null);
  ...
  const filteredTvRecomendationData = await tvRecomendationData.results.filter((items) => items.backdrop_path !== null && items.poster_path !== null);
