...
export default function MovieDetail(props) {
  ...
  const {
    movieDetailData, movieVideoData, movieCreditData, movieSimilarData,
  } = props;
...
export const getServerSideProps = async (paths) => {
  ...
  const filteredMovieVideoData = await movieVideoData.results.filter((items) => items.site === 'YouTube');
  ...
  const filteredMovieCreditData = await movieCreditData.cast.filter((items) => items.known_for_department === 'Acting' && items.profile_path !== null);
  ...
  const filteredMovieSimilarData = await movieSimilarData.results.filter((items) => items.backdrop_path !== null && items.poster_path !== null);
  ...
  const filteredMovieRecomendationData = await movieRecomendationData.results.filter((items) => items.backdrop_path !== null && items.poster_path !== null);
