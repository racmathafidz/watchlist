...
export default function Home(props) {
  const {
    allTrending, movieTrending, tvTrending, moviePopular, tvPopular, movieInTheater, tvAiring,
  } = props;

  // Get 5 Movie / Tv Show for Carousel
  const allTrendingSlide = allTrending.filter((items, index) => index <= 4);
...
export const getServerSideProps = async () => {
  ...
  const filteredAllTrendingData = allTrending.results.filter((items) => items.backdrop_path !== null && items.poster_path !== null);
